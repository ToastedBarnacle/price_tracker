...
def load_data_files(previous_file):
    ...
    if previous_file not in data_files:
        st.error(f"Invalid selected file: {previous_file}. Please select a valid previous data set.")
        st.stop()
...
def calculate_trends(newest_df, previous_df, filters):
    ...
    for col in ['loose-price', 'psa-10-price', 'sales-volume']:
        trend_data[f"{col}-change"] = ((trend_data[f"{col}_new"] - trend_data[f"{col}_old"]) / trend_data[f"{col}_old"]) * 100
...
def render_trends_page(filters):
    ...
    try:
        ...
        # Render trend tables
        render_table("Top 10 Cards by Loose Price Change", "loose-price-change", "loose-price-change", ['loose-price_old', 'loose-price_new'])
        render_table("Top 10 Cards by PSA 10 Price Change", "psa-10-price-change", "psa-10-price-change", ['psa-10-price_old', 'psa-10-price_new'])
        render_table("Top 10 Cards by Sales Volume Change", "sales-volume-change", "sales-volume-change", ['sales-volume_old', 'sales-volume_new'])
...
filters = {
    ...
    "selected_sets": st.sidebar.multiselect("Select Sets", options=get_data_files(), default=[]),
}
